...
{
    public partial class NewsMgt : SetupBasePage
    {
        public const string PageUrl = @"/WebPages/News/NewsMgt.aspx";
        ...
        private IEnumerable<NewsDto> _currentInstances;
        public IEnumerable<NewsDto> CurrentInstances
        {
            get
            {
                if (_currentInstances == null && IsInSession(UniqueInstancesStateKey))
                {
                    _currentInstances = GetFromSession(UniqueInstancesStateKey) as IEnumerable<NewsDto>;
                }
        ...
        protected void ucIList_InstanceRowDeleting(object sender, InstanceRowDeletingEventArgs e)
        {
            ...
            {
                NewsFacade facade = new NewsFacade(uow);
                IFacadeUpdateResult<NewsData> result = facade.DeleteNews(e.Instance.Id);
        ...
        protected void ucIList_InstanceRowSaving(object sender, InstanceRowSavingEventArgs e)
        {
            using (IUnitOfWork uow = UnitOfWorkFactory.Instance.Start(DataStoreResolver.CRMDataStoreKey))
            {
                NewsDto project = e.Instance as NewsDto;
                NewsFacade facade = new NewsFacade(uow);
                IFacadeUpdateResult<NewsData> result = facade.SaveNews(project);
                e.IsSuccessful = result.IsSuccessful;
        ...
        private void RetrieveInstances(NewsFacade facade)
        {
            if (CurrentUserContext.IsAdmin || CurrentUserContext.IsEmployee)
            {
                CurrentInstances = facade.RetrieveAllNews(new NewsConverter());
            }
        }
    }
}